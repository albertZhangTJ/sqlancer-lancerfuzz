...
public class Fuzzer {
    ...
    private HashMap<String, String> static_var_ref;
    ...
    public Fuzzer(SQLConnection con, int fuzzing_depth, int loop_threshold){
        ...
        this.static_var_ref = new HashMap<>();
...
//STAGE_CALL_RULE
    ...
    private String get_static_variable(String key){
        return this.static_var_ref.get(key);
    }
    ...
    private void set_static_variable(String key, String value){
        this.static_var_ref.put(key, value);
    }
...
//IT IS THE TESTER'S RESPONSIBILITY TO ENSURE THE PROPER PARAMETERS ARE SPECIFIED IN THE GRAMMAR FILE
    ...
    private String tableName(int depth, String iid, boolean is_new, String sub, String sup) throws Exception{
        ...
        if (is_new){
            ...
            List<String> values = new ArrayList<>();
            while (rs.next()){
                if (iid==null || this.is_identifier_available(iid, rs.getString("Tables_in_" + parent_name + "") )){
                    values.add(rs.getString("Tables_in_" + parent_name + ""));
                }
            }
            if (values.size()==0){
                throw new Exception("IgnoreMe: quantifier node multiplier cannot be satisfied");
...
//ALTERNATION_NODE_SUB_OPTION
            if (r < 1.0){
                ...
                String temp = Node80(depth-1);
                if (temp!=null){
                    if (false){
                        if (false){
                            this.set_static_variable("", temp);
                        }
                        else {
...
//ALTERNATIVE_NODE
//MIN_DEPTH = 2
    public String Node80(int depth) throws Exception{

        String ans=null;
        if (false){
            if (true){
                ans = this.get_static_variable("db");
            }
            else {
...
//ACTION_NODE
//MIN_DEPTH = 1
    public String Node89(int depth){
...
//LITERAL_NODE
//MIN_DEPTH = 1
    public String Node122(int depth){
...
//ACTION_NODE
//MIN_DEPTH = 1
    public String Node126(int depth){
...
//LITERAL_NODE
//MIN_DEPTH = 1
    public String Node128(int depth){
...
//CHARSET_NODE
//MIN_DEPTH = 1
    public String Node151(int depth) throws Exception{
