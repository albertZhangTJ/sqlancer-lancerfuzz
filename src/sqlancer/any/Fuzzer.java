...
//STAGE_CALL_RULE
            if (index==0){
                String stmt = dropDatabase(fuzzing_depth);
                System.out.println(stmt);
                this.test_case_buffer.add(stmt);
...
//STAGE_CALL_RULE
            if (index==0){
                String stmt = createDatabase(fuzzing_depth);
                System.out.println(stmt);
                this.test_case_buffer.add(stmt);
                con.createStatement().execute(stmt);
            }
...
//STAGE_CALL_RULE
            if (index==0){
                String stmt = useDatabase(fuzzing_depth);
                System.out.println(stmt);
                this.test_case_buffer.add(stmt);
                con.createStatement().execute(stmt);
            }
...
//STAGE_CALL_RULE
            if (index==0){
                String stmt = createTable(fuzzing_depth);
                System.out.println(stmt);
                this.test_case_buffer.add(stmt);
                con.createStatement().execute(stmt);
            }
...
//STAGE_CALL_RULE
            if (index==0){
                String stmt = insertStatement(fuzzing_depth);
                System.out.println(stmt);
                this.test_case_buffer.add(stmt);
                con.createStatement().execute(stmt);
            }
...
//STAGE_CALL_RULE
            if (index==0){
                String stmt = updateStatement(fuzzing_depth);
                System.out.println(stmt);
                this.test_case_buffer.add(stmt);
                con.createStatement().execute(stmt);
            }
...
//UNPARSER_CALL_CHILDREN
            ...
            if (cache.size()<=13 || cache.get(13).indexOf(ERROR_FLAG)!=-1){
                this.snapshot_used_identifier();
                int eeb_size = this.expected_error_buffer.size();
                String tmp = Node115(depth-1);
                if (cache.size()<=13){
                    cache.add(tmp);
                }
                else {
            ...
            if (cache.size()<=14 || cache.get(14).indexOf(ERROR_FLAG)!=-1){
                this.snapshot_used_identifier();
                int eeb_size = this.expected_error_buffer.size();
                String tmp = SC(depth-1);
                if (cache.size()<=14){
...
//LITERAL_NODE
//MIN_DEPTH = 1
    public String Node118(int depth){
